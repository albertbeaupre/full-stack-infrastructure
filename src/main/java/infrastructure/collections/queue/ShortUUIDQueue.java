package infrastructure.collections.queue;
 ...
 * short indexes efficiently, reusing them when possible. It provides methods to pop an
...
public class ShortUUIDQueue {
    ...
    /**
     * Pops an index from the queue, returning a unique short index.
    ...
    public short pop() {
        if (enqueue > 0) {
            ...
            short oldIndex = this.queue[index];
    ...
    public void push(short index) {
        ...
        if (enqueue >= queue.length) {
            short[] newArray = new short[queue.length * 2];
